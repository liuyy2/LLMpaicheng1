...
class TRCGRepairPolicy(BasePolicy):
    ...
    def decide(
        ...
        _severity = str(repair_profile.get('severity_level', '') or '')

        # Respect a valid LLM decision. Only do a tiny neighborhood expansion
        # when the model chose nothing, or when a singleton root is too narrow
        # for a clearly high-pressure cluster.
        if decision_source.startswith("llm"):
            if len(_unlock_set) == 0:
                _target_unlock = 1
            elif len(_unlock_set) == 1 and _severity in ("high", "critical"):
                _target_unlock = 2
            else:
                _target_unlock = len(_unlock_set)

        if decision_source == "heuristic_fallback":
            _target_unlock = min(_target_unlock, max(len(_unlock_set), 3))
    ...
    def _build_repair_profile(
        ...
        # Important: widen the local search space first, but keep the initial
        # anchor strong. If freeze/epsilon are relaxed too aggressively up
        # front, the policy wins a little delay and loses a lot of stability.
        # The fallback chain is already responsible for progressively relaxing
        # freeze/epsilon when the anchored solve is infeasible.
        target_unlock = 2
        max_ratio = 0.20
    ...
    def _build_skip_return(
        ...
        eligible_ids = {
            ...
            for item in trcg_dict.get('urgent_missions', [])
            if str(item.get('mission_id', '') or '')
        }
        profile = self._build_repair_profile(trcg_dict, len(eligible_ids), config)
        freeze_h_slots = int(profile['freeze_h_slots'])
        epsilon_solver = float(profile['epsilon_solver'])
        ...
        max_unlock = int(profile['target_unlock'])
    ...
    def _compute_state_fingerprint(
        ...
        conflict_keys = sorted(
            (
                c.get('mission_a', c.get('a', '')),
                c.get('mission_b', c.get('b', '')),
                c.get('resource', ''),
            )
            for c in conflicts
        ) if conflicts else []
    ...
    def _build_stable_skip_return(
        ...
        eligible_ids = active_mission_ids - started_mission_ids - completed_mission_ids
        profile = self._build_repair_profile(trcg_dict, len(eligible_ids), config)
        _unlock_set = set(valid_unlock)
        _target_unlock = int(profile['target_unlock'])
        _max_unlock = int(profile['max_unlock'])
        _severity = str(profile.get('severity_level', '') or '')

        if prev_source.startswith("llm"):
            if len(_unlock_set) == 0:
                _target_unlock = 1
            elif len(_unlock_set) == 1 and _severity in ("high", "critical"):
                _target_unlock = 2
        ...
        if len(_unlock_set) < _target_unlock:
            ...
            for c in sorted(conflicts, key=lambda x: -float(x.get('severity', 0))):
                for m in (
                    ...
                    if m in eligible_ids and m not in _unlock_set and len(_unlock_set) < _max_unlock:
                        _unlock_set.add(m)
                if len(_unlock_set) >= _target_unlock:
                    break
        ...
        if len(_unlock_set) < _target_unlock:
            ...
            for u in sorted(urgents, key=lambda x: x.get('urgency_score', 9999)):
                mid = u.get('mission_id', '')
                if mid in eligible_ids and mid not in _unlock_set and len(_unlock_set) < _max_unlock:
                    _unlock_set.add(mid)
                if len(_unlock_set) >= _target_unlock:
                    break
        ...
        freeze_h_slots = int(profile['freeze_h_slots'])
        epsilon_solver = max(
            float(profile['epsilon_solver']),
            float(prev_decision.epsilon_solver),
        )
        ...
        logger.info(
            "t=%d source=skip_stable_state (reuse prev %s decision) wall=%dms",
            now, prev_source, wall_ms,
        )
